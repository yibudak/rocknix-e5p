        ...
        int (*handle_line)(char*, struct mipi_dsi_device*, struct generic_panel*))
{
    ...
    if (strlen(descfile) > 0) {
        ...
        ctx->no_post_init_cmds =
            of_property_read_bool(dev->of_node, "rocknix,no-post-init-cmds");
        if (linescnt < 1) {
...
static int generic_panel_prepare(struct drm_panel *panel)
{
    ...
    /*
     * E5P (GameMT E5 Plus): the panel init sequence already ends with
     * exit_sleep(120ms) + display_on(20ms) in the correct order. Resending
     * the pair below (display-on THEN sleep-out, i.e. reversed) makes the
     * last command on the wire a sleep-out, which restarts the DDIC's
     * internal TCON and locks the panel to a wrongly phased frame origin
     * (constant vertical shift of vsa+vbp lines, wrap-around). Panels whose
     * init blob handles 0x11/0x29 itself set "rocknix,no-post-init-cmds".
     */
    if (!ctx->no_post_init_cmds) {
